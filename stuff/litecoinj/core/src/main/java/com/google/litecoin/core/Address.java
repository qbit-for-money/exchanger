 ...
 * are about network (TCP) addresses.</p>
 *
 * <p>
 ...
 * unsupported type) can contain a hash of a script instead.</p>
 */
public class Address extends VersionedChecksummedBytes {
	...
	/**
	 * Construct an address from parameters, the address version, and the
	 * hash160 form. Example:
	 * <p>
	 *
	...
	/**
	 * Returns an Address that represents the script hash extracted from the
	 * given scriptPubKey
	 */
	...
	/**
	 * Construct an address from parameters and the standard "human
	 * readable" form. Example:
	 * <p>
	 *
	 * <pre>new Address(NetworkParameters.prodNet(), "17kzeh4N8g49GFvdDzSf8PjaPfyoD1MndL");</pre><p>
	 *
	 * @param params The expected NetworkParameters or null if you don't
	 * want validation.
	 * @param address The textual form of the address, such as
	 * "17kzeh4N8g49GFvdDzSf8PjaPfyoD1MndL"
	 * @throws AddressFormatException if the given address doesn't parse or
	 * the checksum is invalid
	 * @throws WrongNetworkException if the given address is valid but for a
	 * different chain (eg testnet vs prodnet)
	 */
	...
	/**
	 * Examines the version byte of the address and attempts to find a
	 * matching NetworkParameters. If you aren't sure which network the
	 * address is intended for (eg, it was provided by a user), you can use
	 * this to decide if it is compatible with the current wallet. You
	 * should be able to handle a null response from this method. Note that
	 * the parameters returned is not necessarily the same as the one the
	 * Address was created with.
	 *
	 ...
	 * matching NetworkParameters. If you aren't sure which network the
	 * address is intended for (eg, it was provided by a user), you can use
	 * this to decide if it is compatible with the current wallet.
	 *
	 * @return a NetworkParameters or null if the string wasn't of a known
	 * version.
	 */
	@Nullable
	...
	/**
	 * Check if a given address version is valid given the
	 * NetworkParameters.
	 */
